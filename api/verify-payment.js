...
export default async function handler(req,res){
 ...
 try{
  ...
  return res.status(200).json({success:true,message:"Payment verified successfully",paymentMode,testOnly:paymentMode==="test",downloadUrl:`/api/download?paperId=${encodeURIComponent(paperId)}&token=${encodeURIComponent(token)}`});
